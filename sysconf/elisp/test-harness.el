...
BODY is a sequence of expressions and may contain several tests."
	...
	`(condition-case error-info
	     (progn
	       ...
	       (incf passes))
	   ((error cl-assertion-failed)
            (kill-ring-save (point-min) (point-max))
	    (Print-Failure "Assertion failed:\n%S\nGOT Region:\n%s" (quote ,assertion) (car kill-ring))
	  ...
	  (let ((quoted-body (if (= 1 (length body))
	    ...
	    `(let ((messages ""))
		 ...
		 (let ((msg-string (apply 'format (ad-get-args 0))))
		   (setq messages (concat messages msg-string)) ; messages is void here -blakem
		   msg-string))
	       (condition-case error-info
		   (progn
		     (setq trick-optimizer (progn ,@body))
...
(defun batch-test-emacs-1 (file)
  (condition-case error-info
    ...
    (error
     ...
     (princ file)
     (princ ": ")
     (princ (format "display-error: %s" error-info))
...
(defun clear-buffer() (kill-region 1 (point-max)))

(defmacro with-cperl-buffer (&rest body)
  `(with-temp-buffer (cperl-mode) ,@body)
)
