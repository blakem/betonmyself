...
;; $Id: cperl-mode.el 4844 2004-10-07 09:38:31Z blakem $
...
(defcustom cperl-auto-newline-after-brace nil
  "*Non-nil means automatically newline even after closing brace.
Subject to `cperl-auto-newline' setting."
  :type 'boolean
  :group 'cperl-autoinsert-details)
		 ...
		 (or (and (null only-before)
		     ...
		     (and (eq last-command-char ?\{) ; Do not insert newline
 		     ...
 		     (if (and cperl-auto-newline
                              cperl-auto-newline-after-brace)
			 (progn (cperl-indent-line) (newline) t) nil)))
	    (progn
	      (self-insert-command (prefix-numeric-value arg))
	      (cperl-indent-line)
	      (if (and cperl-auto-newline cperl-auto-newline-after-brace)
		  (setq insertpos (1- (point))))
	      (if (and (and cperl-auto-newline cperl-auto-newline-after-brace)
                       (null only-before))
		  (progn
		    (newline)
		    (cperl-indent-line)))
	...
	(let* ((case-fold-search nil)
	  ...
	  (cond ((get-text-property (point) 'indentable)
		 ...
		 (or (memq char-after (append ")]}" nil))
		     ...
		     (skip-chars-forward " \t"))
                        (progn
                           ...
                           (beginning-of-line)
                           (skip-chars-forward " \t")
                           (+ cperl-indent-level (current-column))) ;qw -blakem
                  )
		...
		((null containing-sexp)
		 ...
		 (+ (save-excursion
		      (goto-char start)
		      (- (indentation-for-line-after-curly) ;qw -blakem
		 ...
		 (or (memq char-after
		     ...
		     (skip-chars-forward " \t"))
                     ...
                     (progn (goto-char containing-sexp)
                                     ...
                                     (+ (current-column) 
                                        cperl-indent-level
                                        (if (looking-at "if ") 2 0)
                                        (if (looking-at "unless ") 2 0)
                                        (if (looking-at "while ") 2 0)
                     ))) ; -blakem ( tweak for if )
		     ...
		     (progn
		       ...
		       (+ (if (memq char-after (append "}])" nil))
			      (progn (goto-char containing-sexp) 
                                     (beginning-of-line) 
                                     (skip-chars-forward " \t") 
                                     (current-column)) ; Closing paren -blakem
			    cperl-continued-statement-offset)
			  (if (or is-block
				  (not delim)
				  (not (eq char-after ?\})))
...
; (defun indentation-for-line-after-curly () (current-indentation))
(defun indentation-for-line-after-curly () ;qw -blakem
  (let ((currindent (current-indentation)))
    (save-excursion
