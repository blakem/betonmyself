...
;;; @(#) $Id: tinylibm.el 2138 2003-05-19 02:37:26Z blakem $
...
;; LCD Archive Entry:
;; tinylibm|Jari Aalto|jari.aalto@poboxes.com|
;; Library of small macros and functions.|
;; 2001-12-31|$Revision: 2138 $|~/misc/tinylibm.el.Z|
...
(require 'tinylibb)			;Backward compatible fucntions
...
(defun tinylibm-version (&optional arg)
  ...
  (interactive "P")
  (ti::m-pkg-version-info "tinylibm.el" arg))
...
Float time value in seconds is sent to FUNCTION.
  ...
  (`
   (let* ((Time-A (current-time))
     ...
     (setq Time-Diff (ti::d-time-difference Time-B Time-A 'float))
...
TYPE
    ...
    packages be backward compatible you want to call this fucntions prior
...
(defun ti::re-search-check (re &optional level start-form read)
  "Check whole buffer if we find regexp RE.
...
(defsubst ti::non-dedicated-frame (&optional win)
  ...
  (if (window-dedicated-p (selected-window))
      (car (ti::w-frame-list nil nil win))
    ;;  current frame
...
E.g. you can't call `find-file', `switch-to-buffer' in dedicated frame."
  (if (window-dedicated-p (selected-window))
      (raise-frame (select-frame (car (ti::w-frame-list))))))
...
;;; ----------------------------------------------------------------------
;;;
(defmacro ti::defun-byte-compiled-p  (function-symbol)
...
If you have highly important fucntions that must be as fast as possible
...
;;; ----------------------------------------------------------------------
;;;
(defmacro ti::use-dynamic-compilation ()
  "Turn on dynamic cmpilation in this package.
You add this staement to the beginning of your file:

   (eval-when-compile (ti::use-dynamic-compilation))"
  (`
   (progn
...
;;; ----------------------------------------------------------------------
;;;
(defmacro ti::use-package-emacs (emacs xemacs &rest body)
  "EMACS and XEMACS package compatibility and evaluate BODY.
E.g. `timer' in Emacs and 'itimer in XEmacs
Recommended usage: (eval-and-compile (ti::use-package-emacs ...))."
  (`
   (progn
     (if (emacs-p)
...
** Tinylibm: Visit ftp://ftp.cs.umb.edu/pub/misc/. Cannot deduce to jka-compr,
...
** Tinylibm: compatible with jka-compr. (1999-02 up till Emacs 20.3"))
      ...
      (t				;Last chance
	   ...
	   (jka-compr-install))))))	;New X/Emacs releases need this
...
Remember:
  ...
  `Add-hook' creates a hook variable if it doesn't exist.
...
;;; ----------------------------------------------------------------------
;;;
(defmacro ti::m-load-user-init-file ()
...
;;; ----------------------------------------------------------------------
;;;
(defmacro ti::m-Info-directory-list ()
  "Emacs and XEmacs compatibility."
...
(defsubst ti::file-name-forward-slashes-cygwin (file)
  "Convert Win32 F:\\filename to /cygdrive/LETTER/filename syntax."
  (when file
    (setq file (ti::file-name-forward-slashes file))
    (while (when (string-match "\\(\\([a-z]\\):\\)\\([\\/].*\\)" file)
	     (setq file (replace-match (concat "/cygdrive/" (match-string 2 file))
				       nil nil file 1))))
    file))
...
Input:
  ...
  (let (dir)
    (when find-temp-dir
      ...
      (setq file (ti::f-make-path dir (file-name-nondirectory file))))
...
;;;
(put 'ti::defgroup 'lisp-indent-function 3)
(defmacro ti::defgroup (symbol prefix group &optional doc)
     ...
     (, doc)
       ...
       :link '(link
	       ...
	       :func-args (list
			   ...
			   (, symbol))
	     :action    ti::m-defgroup-mail))))

;;; ----------------------------------------------------------------------
;;; This would actually belong to ti::defgroup
;;;
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::m-defgroup-mail (widget &rest ignore)
...
name `xxx-submit-bug-report' is derived."
...
(defmacro ti::momentary-output-macro (buffer &optional echo-msg win1 &rest body)
...
Input:
  ...
  version (OS) and EEMACS version. Supply non-nil (t) values for these if you
  need exacly a specific file for Win32/Unix and for XEmacs/Emacs."
...
Example usage:
  ...
  (` (progn
	 ...
	 (load "overlay" 'noerr))      ;; has no provide statement
       (or (fboundp 'overlays-at) ;; Did it define this fucntion?
	   (progn
	    (,@ body))))))
