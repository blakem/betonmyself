...
;;; @(#) $Id: tinylibb.el 2138 2003-05-19 02:37:26Z blakem $
...
;; LCD Archive Entry:
;; tinylibb|Jari Aalto|jari.aalto@poboxes.com|
;; library of (b)ackward compatible functions.|
;; 2001-12-20|$Revision: 2138 $|~/misc/tinylibb.el.Z|
...
(defun-maybe make-local-hook (hook) ;; Exists in 19.30+
  ...
  ;; - I need locals so many times it make sme cry, eg. post-command-hook
...
Input:
  ...
  LEVEL		The submatch in REGEXP to end reading substring.
		Default is 0
  CONT-LEVEL	The submatch end to continue reading the STRING.
...
;;  "Read match from buffer atsubmatch LEVEL. Optionally from STRING.
...
(eval-and-compile
  (if (locate-library "executable") ;; 20.4 defines this
    ...
    (defun-maybe executable-find (program-name)
      ...
      (ti::f-get-load-path program-name exec-path))))
   ...
   (when (or (not (fboundp (, function1)))
	     (and (emacs-p)
		  ...
		  (or (not (equal (symbol-function (, function1))
				  (, function2)))
		      ;;  If the definition is 'ignore, reassign correct function.
...
(defun-maybe count-char-in-region  (beg end char)
  "In region BEG END, count all CHAR occurrances.
Eg to have real line count in buffer that
...
(eval-and-compile
  (when (or (featurep 'xemacs)
    ...
    ;;   defun-maybe. If this function already exists, this autolaod
    ;;   definition is no-op.
    (autoload 'subst-char-in-string "tinylibb.el")))


;; Emacs and XEmacs differ here. Convert Emacs func --> XEmacs name
...
;;; ...................................................... &func-tests ...
;;; We define these here becasue they are used lated in this library
;;; "define before using"
...
Notes:
  if function is alias, the real function behing it is examined.
  ...
  (let* ((args-re "[^(]+\\([^)]+)\\)")
	...
	;;  since it would load the package unnnecessarily
...
(defun tinylibb-version (&optional arg)
  ...
  (interactive "P")
  (ti::m-pkg-version-info "tinylibb.el" arg))
